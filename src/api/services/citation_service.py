    ...
    try:
        if cluster_id:
            ...
            for _ in range(depth):
                ...
                for opinion in current_level:
                    new_citing = set(
                        opinion.cited_by.all()[: limit // len(current_level)]
                    )
            ...
            court_opinions = get_filtered_opinions(court_id=court_id, limit=limit)
...
def get_citation_stats(
    ...
    try:
        ...
        opinions = get_filtered_opinions(
            court_id=court_id, limit=None, **filters  # No limit for stats calculation
        )
        ...
        for opinion in opinions:
            ...
            for cited in citations:
                rel = opinion.cites.relationship(cited)
                if rel.treatment:
                    treatment_counts[rel.treatment] = (
                        treatment_counts.get(rel.treatment, 0) + 1
                    )
                if rel.relevance:
                    relevance_distribution[rel.relevance] = (
                        relevance_distribution.get(rel.relevance, 0) + 1
                    )
                if rel.opinion_section:
                    section_distribution[rel.opinion_section] = (
                        section_distribution.get(rel.opinion_section, 0) + 1
                    )
        ...
        return {
            ...
            "avg_citations_per_opinion": (
                total_citations / total_opinions if total_opinions > 0 else 0
            ),
...
def get_influential_citations(
    ...
    try:
        ...
        top_cited = sorted(
            citation_counts.items(), key=lambda x: x[1][1], reverse=True
        )[:limit]
...
def _format_citation_relationship(self, rel, node) -> dict:
    """Format a citation relationship for the API response."""
    return {
        "cluster_id": node.cluster_id,
        "case_name": node.case_name,
        "date_filed": node.date_filed,
        "court_id": node.court_id,
        "court_name": node.court_name,
